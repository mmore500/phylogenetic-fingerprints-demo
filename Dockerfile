...
COPY . /opt/phylogenetic-fingerprints-demo

RUN \
  pip3 install -r /opt/phylogenetic-fingerprints-demo/docs/requirements.txt \
    && \
...
RUN \
  chgrp --recursive user /opt/phylogenetic-fingerprints-demo \
    && \
  chmod --recursive g+rwx /opt/phylogenetic-fingerprints-demo \
    && \
  echo "user granted permissions to /opt/phylogenetic-fingerprints-demo"

USER user

RUN \
  cd /opt/phylogenetic-fingerprints-demo \
    && \
...
WORKDIR /opt/phylogenetic-fingerprints-demo
